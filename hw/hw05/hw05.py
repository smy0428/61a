...
class VendingMachine:
    ...
    def deposit(self, money):
        self.balance += money
        if self.stock == 0:
            self.balance, refund = 0, self.balance
            return 'Machine is out of stock. Here is your ${}.'.format(refund)
        elif self.stock > 0:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            if self.balance < self.price:
                ...
                self.balance, refund, self.stock = 0, self.balance - self.price, self.stock - 1
...
def preorder(t):
    ...
    """
    numbers, child = [], []
    numbers.append(t.label)
    if t.branches != []:
        for branch in t.branches:
            child += preorder(branch)
    return numbers + child
...
def store_digits(n):
    ...
    while n >= 10:
        n, a = n // 10, n % 10
        """s.first, s.rest = a, Link(s.first, s.rest)"""
        s = Link(a, s)
    s = Link(n, s)
    return s
...
def deep_map(f, link):
    ...
    if link is Link.empty:
        ...
        if type(link.first) == Link:
            link_final = Link(deep_map(f, link.first), deep_map(f, link.rest))
        elif type(link.first) == int:
            link_final = Link(f(link.first), deep_map(f, link.rest))
        return link_final
