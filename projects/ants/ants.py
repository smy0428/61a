...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        pa = self.place
        i = 0
        while i < self.min_range or pa.bees == []:
            i += 1
            pa = pa.entrance
            if pa == beehive:
                return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor > amount:
            for i in lst:
                self.place.bees[self.place.bees.index(i)].reduce_armor(amount)
            self.armor -= amount
            self.place.bees
        else:
            for i in lst:
                self.place.bees[self.place.bees.index(i)].reduce_armor(self.armor + self.damage)
            self.armor = 0
            self.place.remove_insect(self)
            self.death_callback()
        # END Problem 5
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.True_Queen:
            ...
            self.Queen = False
        QueenAnt.True_Queen.append(self)
    ...
    def action(self, colony):
        ...
        if self.Queen:
            ...
            while start_place.exit:
                start_place = start_place.exit
                if start_place.ant:
                    if not start_place.ant.buffed:
                        start_place.ant.damage *= 2
                        start_place.ant.buffed = True
                    if start_place.ant.is_container:
                        if start_place.ant.contained_ant:
                            if not start_place.ant.contained_ant.buffed:
                                start_place.ant.contained_ant.damage *= 2
                                start_place.ant.contained_ant.buffed = True
...
def apply_effect(effect, bee, duration):
    ...
    saved_action = bee.action
    bee.action = effect(saved_action, bee)"""
            ...
            if ant and ant != self:
                if ant.is_container:
                    ...
                    if ant.contained_ant:
                        dict[ant.contained_ant] = distance
                else:
                    dict[ant] = distance
            for bee in bees:
                dict[bee] = distance
